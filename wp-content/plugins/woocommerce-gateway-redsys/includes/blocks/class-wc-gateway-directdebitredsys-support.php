<?php
/**
 * WC_Gateway_Directdebitredsys_Support class file.
 *
...
final class WC_Gateway_Directdebitredsys_Support extends AbstractPaymentMethodType {
	...
	public function get_payment_method_script_handles() {
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'wc-directdebitredsys-payments-blocks', 'woocommerce-redsys', REDSYS_PLUGIN_PATH_P . 'languages/' );
		}
	...
	public function get_payment_method_data() {
		if ( ! empty( WCRed()->get_redsys_option( 'logo', 'directdebitredsys' ) ) ) {
			...
			$icon     = apply_filters( 'woocommerce_directdebitredsys_icon', $logo_url );
		} else {
			$icon = apply_filters( 'woocommerce_directdebitredsys_icon', REDSYS_PLUGIN_URL_P . 'assets/images/redsys.svg' );
		}
		if ( '' === $icon ) {
			$icon = false;
		}
