...
if ( ! class_exists( 'Redsys_Advanced_Settings' ) ) {
	...
	class Redsys_Advanced_Settings {
		...
		public static function get_settings( $section = null ) {
			...
			WCRed()->log( 'redsys-settings', 'saved_shipping_methods: ' . print_r( $saved_shipping_methods, true ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
			...
			WCRed()->log( 'redsys-settings', 'options_shipping: ' . print_r( $options_shipping, true ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
			...
			switch ( $section ) {
				...
				case 'sequential_invoice':
					$settings = array(
						...
						array(
							...
							'type' => 'text',
							// translators: %1$s and %2$s are the opening and closing <a> tags, %3$s is the link to the documentation.
				...
				case 'qr_codes':
					$settings = array(
						...
						array(
							'title'       => esc_html__( 'User plugins.joseconti.com', 'woocommerce-redsys' ),
							'type'        => 'text',
							'description' => esc_html__( 'User plugins.joseconti.com', 'woocommerce-redsys' ),
							'default'     => '',
				...
				default:
					$settings = array(
						...
						array(
							'title'   => esc_html__( 'Enable Order Cancellation', 'woocommerce-redsys' ),
							'type'    => 'checkbox',
							'label'   => esc_html__( 'Enable Order Cancellations.', 'woocommerce-redsys' ),
							'default' => 'no',
							'desc'    => esc_html__( 'Enable Order Cancellation', 'woocommerce-redsys' ),
							'id'      => 'redsys_cancel_enabled',
						),
						array(
							'title'   => esc_html__( 'Cancellation Time (in seconds)', 'woocommerce-redsys' ),
							'type'    => 'number',
							'desc'    => esc_html__( 'Time in seconds for automatic cancellation of pending orders.', 'woocommerce-redsys' ),
							'default' => '1200',
							'id'      => 'redsys_cancel_time',
						),
						array(
							'title'   => esc_html__( 'Disable logos at Checkout', 'woocommerce-redsys' ),
							'type'    => 'checkbox',
							'label'   => esc_html__( 'Disable Payment Method logos at Checkout.', 'woocommerce-redsys' ),
							'default' => 'no',
							'desc'    => esc_html__( 'Disable Payment Method logos at Checkout.', 'woocommerce-redsys' ),
							'id'      => 'redsys_disable_logos_checkout',
						),
						array(
							'title'    => esc_html__( 'Logo Display Option', 'woocommerce-redsys' ),
							'type'     => 'select',
							'desc'     => esc_html__( 'Choose how the logo is displayed with the payment method title.', 'woocommerce-redsys' ),
							'id'       => 'redsys_logo_display_option',
							'default'  => 'left',
							'options'  => array(
								'left'      => esc_html__( 'Left of Text', 'woocommerce-redsys' ),
								'right'     => esc_html__( 'Right of Text', 'woocommerce-redsys' ),
								'afterText' => esc_html__( 'After Text', 'woocommerce-redsys' ),
								'iconOnly'  => esc_html__( 'Icon Only', 'woocommerce-redsys' ),
							),
							'desc_tip' => true,
							'class'    => 'redsys-logo-display-option',
						),
						array(
							'type' => 'sectionend',
							'id'   => 'general_options',
 ...
 */
function redsys_add_invoice_number( $columns ) { // phpcs:ignore Universal.Files.SeparateFunctionsFromOO.Mixed

	$new_column = ( is_array( $columns ) ) ? $columns : array();
	if ( isset( $new_column['wc_actions'] ) ) {
 ...
 * Invoice Pattern.
 *
 * @param string $string_text String.
 */
function redsys_use_patterns( $string_text ) {
	$numericzero                   = preg_replace( '/(\{d\})/', date_i18n( 'd' ), $string_text );
...
function redsys_check_current_year() {
	...
	} elseif ( $current_year > $saved_year ) {
...
if ( 'yes' === get_option( 'redsys_disable_logos_checkout' ) ) {
	add_filter( 'woocommerce_redsys_icon', '__return_empty_string' );
	add_filter( 'woocommerce_applepayredsys_icon', '__return_empty_string' );
	add_filter( 'woocommerce_bizumcheckout_icon', '__return_empty_string' );
	add_filter( 'woocommerce_bizumredsys_icon', '__return_empty_string' );
	add_filter( 'woocommerce_directdebitredsys_icon', '__return_empty_string' );
	add_filter( 'woocommerce_googlepayredsys_icon', '__return_empty_string' );
	add_filter( 'woocommerce_googlepayredirecredsys_icon', '__return_empty_string' );
	add_filter( 'woocommerce_insite_icon', '__return_empty_string' );
	add_filter( 'woocommerce_masterpass_icon', '__return_empty_string' );
	add_filter( 'woocommerce_paygold_icon', '__return_empty_string' );
	add_filter( 'woocommerce_bank_redsys_icon', '__return_empty_string' );
}
