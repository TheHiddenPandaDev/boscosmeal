...
class WC_Gateway_Redsys_Scheduled_Actions {
	...
	public function send_expired_credit_card_email_action() {
		...
		if ( ! empty( $results ) ) {
			...
			foreach ( $results as $result ) {
				...
				$subject  = esc_html__( 'Your credit card expires this month at ', 'woocommerce-redsys' ) . get_bloginfo( 'name' );
				$message  = '<html><body>';
				$message .= esc_html__( 'Dear ', 'woocommerce-redsys' ) . $nombre . ' ' . $apellido . ',<br />';
				$message .= esc_html__( 'Your payment credit card expires this month. Please update your payment information to continue enjoying our services.', 'woocommerce-redsys' ) . '<br />';
				$message .= '<a href="' . esc_url( $payment_methods_url ) . '">' . esc_html__( 'Update your payment information', 'woocommerce-redsys' ) . '</a>';
				$message .= '</body></html>';
				$data     = array(
					'user_id'    => $user_id,
	...
	public function remove_expired_card_action() {
		...
		$this->debug( 'Remove Credit Cards is active' );
		...
		if ( ! empty( $results ) ) {
			...
			foreach ( $results as $result ) {
				...
				$subject  = esc_html__( 'Your credit card has expired at ', 'woocommerce-redsys' ) . get_bloginfo( 'name' );
				$message  = '<html><body>';
				$message .= esc_html__( 'Dear user, You credit card has expired and has been deleted. Please update your payment information to continue enjoying our services.', 'woocommerce-redsys' ) . '<br />';
				$message .= '<a href="' . esc_url( $payment_methods_url ) . '">' . esc_html__( 'Update your payment information', 'woocommerce-redsys' ) . '</a>';
				$message .= '</body></html>';
