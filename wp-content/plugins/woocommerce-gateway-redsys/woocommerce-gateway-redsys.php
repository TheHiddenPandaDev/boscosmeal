...
function woocommerce_gateway_redsys_premium_init() {
	...
	function redsys_add_to_cart_redirect( $checkout_url ) {
		...
		if ( ! is_checkout() && ! is_wc_endpoint_url() && ! is_admin() ) {
	...
	function handle_redsys_capture_order_id() {
		...
		if ( ! empty( $idioma ) ) {
			$data['_billing_idioma_navegador_field'] = sanitize_text_field( wp_unslash( $idioma ) );
		}

		if ( ! empty( $altura_pantalla ) ) {
			$data['_billing_altura_pantalla_field'] = sanitize_text_field( wp_unslash( $altura_pantalla ) );
		}
		...
		if ( ! empty( $profundidad_color ) ) {
			$data['_billing_profundidad_color_field'] = sanitize_text_field( wp_unslash( $profundidad_color ) );
		}

		if ( ! empty( $diferencia_horaria ) ) {
			$data['_billing_diferencia_horaria_field'] = sanitize_text_field( wp_unslash( $diferencia_horaria ) );
		}
 ...
 *
 * @return bool `true` if the cart contains items that require `token_r` payment feature, `false` otherwise.
 */
function redsys_check_cart_for_token_r_requirement() {
	if ( did_action( 'wp_loaded' ) && isset( WC()->cart ) ) {
		...
		if ( ! empty( $contents ) ) {
			...
			foreach ( $contents as $item_key => $item ) {
				...
				if ( WCRed()->need_token_r( $product_id ) ) {
					return true;
				}
				continue;
			}
 ...
 *
 * @return array Lista de características requeridas por los ítems en el carrito.
 */
function redsys_inject_payment_feature_requirements_for_cart_api() {
	if ( redsys_check_cart_for_token_r_requirement() ) {
		return array( 'redsys_token_r' );
	}

	// Si no hay productos en el carrito que requieran `token_r`, no se añade nada.
	return array();
}
/**
 ...
 *
 * @return bool `true` if the cart contains items that require `redsys_preauth` payment feature, `false` otherwise.
 */
function redsys_check_cart_for_preauth_requirement() {
	if ( did_action( 'wp_loaded' ) && isset( WC()->cart ) ) {
		$contents = WC()->cart->get_cart();
		if ( ! empty( $contents ) ) {

			$global_preauth = WCRed()->get_redsys_option( 'redsyspreauthall', 'redsys' );
			...
			foreach ( $contents as $item_key => $item ) {
				$product_id = $item['product_id'];
				if ( WCRed()->need_preauth( $product_id ) ) {
					return true;
				}
				continue;
			}
 ...
 * @return array Lista de características requeridas por los ítems en el carrito.
 */
function redsys_preauth_inject_payment_feature_requirements_for_cart_api() {
	if ( redsys_check_cart_for_preauth_requirement() ) {
		return array( 'redsys_preauth' );
	}
