jQuery( function( $ ) {
	...
	var $shipping_data_container     = $components_panel.parent().find( '#shipping_product_data' ),
		...
		$composite_type_options      = $composite_type_container.find( '.bto_type_options button' ),
		$sold_individually_container = $( '#_sold_individually' ).closest( '.form-field' );

	$composite_type_options.find( '.woocommerce-help-tip' ).attr( 'tabindex', -1 );

	$layout_options = $components_panel.find( '.bto_layouts_list .bto_layout_button' );
	$layout_options.find( '.woocommerce-help-tip' ).attr( 'tabindex', -1 );
	...
	$components_panel
		...
		// Layout selection.
		.on( 'click', '.bto_layout_button', function(e) {
	...
	function Component( $el ) {
		...
		this.initialize_content = function() {
			...
			$components_panel.triggerHandler( 'wc-cp-component-initialized', [ self ] );
		};
	...
	function init_composite_shipping() {
		...
		$composite_type_options.on( 'focus', function() {
			var $option = $( this );
			$composite_type_options.attr( 'tabindex', -1 );
			$option.attr( 'tabindex', 0 );
		});
		$composite_type_options.on( 'keydown', function( e ) {
	...
	function Overlay_Restriction( options ) {
		const self = this;
		const overlay_container_class = 'cp-restriction-section-overlay-container';
		const blurred_class = 'cp-restriction-section-blurred';
		const overlay_class = 'cp-restriction-overlay-upsell';
		const default_options = {
			message: '',
			cta_text: '',
			cta_url: '',
			on_click_cta: null,
			blurred_elements: null,
		...
		this.blur = function() {
			const blurred_elements = self.options.blurred_elements;
			blurred_elements.forEach( function( el ) {
				el.classList.add( blurred_class );
			} );
		};

		this.create_overlay_element = function() {
			const overlay = document.createElement( 'div' );
			overlay.setAttribute( 'class', overlay_class );

			const lock_img = document.createElement( 'img' );
			lock_img.setAttribute( 'src', wc_composite_admin_params.restriction.lock_img_src );
			overlay.appendChild( lock_img );

			if ( self.options.message ) {
				const upsell_text = document.createElement( 'p' );
				upsell_text.textContent = self.options.message;
				overlay.appendChild( upsell_text );
			}

			if ( self.options.cta_text ) {
				const cta = document.createElement( 'a' );
				cta.setAttribute( 'class', 'button button-primary button-large cta' );
				cta.setAttribute( 'target', '_blank' );
				cta.setAttribute( 'href', self.options.cta_url );
				cta.textContent = self.options.cta_text;

				if ( self.options.on_click_cta instanceof Function ) {
					cta.onclick = self.options.on_click_cta;
				}
		...
		this.append_overlay = function() {
			const overlay_containers = self.options.parent_elements;
			overlay_containers.forEach( function( overlay_container ) {
				const overlay = self.create_overlay_element();
				overlay_container.appendChild(overlay);
				overlay_container.classList.add( overlay_container_class )
			} );
		};
	...
	function Notice_Restriction( options ) {
		const self = this;
		const notice_class = 'cp-restriction-notice';
		const default_options = {
			parent_elements: null,
			message: '',
			on_click_cta: null,
			insert_strategy: 'prepend',
			on_show: null,
		};

		this.options = _.defaults( options, default_options );

		this.create_notice_element = function() {
			const div = document.createElement( 'div' );
			div.setAttribute( 'class', notice_class );

			const p = document.createElement( 'p' );
			p.innerHTML = self.options.message;

			const link = p.querySelector( 'a' );
			link.target = '_blank';
			if ( link && self.options.on_click_cta instanceof Function ) {
				link.onclick = self.options.on_click_cta;
			}
		...
		this.show = function() {
			const parent_elements = self.options.parent_elements;
			parent_elements.forEach( function( parent_element ) {
				const notice = self.create_notice_element();
				switch ( self.options.insert_strategy ) {
					case 'append':
						parent_element.append( notice );
						break;
					case 'prepend':
						parent_element.prepend( notice );
						break;
				}
			} );

			if ( self.options.on_show instanceof Function ) {
				self.options.on_show();
			}
		}
	}

	function Select2_Restriction( options ) {
		const self = this;
		const default_options = {
			restricted_option_key: '',
			restricted_option_class: '',
			parent_initialize_event: '',
		...
		this.restrict_item = function( item ) {
			// We disable the restricted option.
			$( item.$content ).find( `option[value="${self.options.restricted_option_key}"]` ).attr( 'disabled', true );

			// Setup the optional popup when the dropdown is opened.
			if ( ! self.options.popup ) {
				return;
			}

			const popup_restriction = new Popup_Restriction({
				attachment_selector: item.$content, // attach it to the content selector.
				message: self.options.popup.message,
				cta_text: self.options.popup.cta_text,
				cta_url: self.options.popup.cta_url,
				on_click_cta: self.options.popup.on_click_cta,
				offset_top: self.options.popup.offset_top,
				offset_left: self.options.popup.offset_left,
				on_show: self.options.popup.on_show,
				on_hide: self.options.popup.on_hide,
				// show the popup when the select2 dropdown is opened.
				// it returns the popup object, and we change top offset on it depending on
				// how the popup is opened (above or below the select).
				setup_show_trigger: function( popup ) {
					$( item.$content ).find( 'select' ).on( 'select2:open', function( e ) {
						if ( $( e.currentTarget ).hasClass( self.options.restricted_option_class ) ) {
							// Set left to the center of the select dropdown
							popup.change_offset_left( 190 + ( $( e.currentTarget ).next( 'span.sw-select2-container' ).innerWidth() / 2 ) + 'px' );

							// Figure out if the popup is opening above or below the select.
							if ( $( e.currentTarget ).next( 'span.sw-select2-container' ).hasClass( 'select2-container--above' ) ) {
								popup.change_offset_top( '0px' );
							} else {
								popup.change_offset_top( '70px' );
							}
							popup.show();
						}
					});
				},
				// hide the popup when the select 2 dropdown is closed.
				setup_hide_trigger: function( popup ) {
					$( item.$content ).find( 'select' ).on( 'select2:close', function( e ) {
						// Delay to allow clicks inside the popup to be registered.
						setTimeout(
							function() {
								// If we are navigating, don't hide the popup.
								if ( popup.is_cta_navigating ) {
									return;
								}
			...
			popup_restriction.restrict();
		}

		this.restrict = function () {
			if ( ! self.options.parent_container ) {
				return;
			}

			self.options.parent_container.on( self.options.parent_initialize_event, function( event, item ) {
				// We don't restrict if they already have it selected.
				if ( self.options.restricted_option_key !== item.$query_type_selector.val() ) {
					self.restrict_item( item );
				}
			});
		}
	}

	function Popup_Restriction( options ) {
		const self = this;
		const overlay_class_name = 'cp-restriction-overlay-upsell-popover';
		const popover_class_name = 'popover-content';
		const close_button_class_name = 'close-btn';
		const default_options = {
			...
			on_show: null,
			on_hide: null,
			setup_show_trigger: null,
			setup_hide_trigger: null,
			on_click_cta: null,
		};
		...
		// is the popup in the middle of navigating the CTA link click?
		this.is_cta_navigating = false;

		this.create_popup_element = function() {
			// Create the main overlay div
			self.overlay = document.createElement( 'div' );
			self.overlay.className = overlay_class_name;
			self.overlay.style.display = 'none';
			self.overlay.style.top = self.options.offset_top;
			self.overlay.style.left = self.options.offset_left;

			// Create the popover content div
			const popover_content = document.createElement( 'div' );
			popover_content.className = popover_class_name;

			// Create the close button
			const close_button = document.createElement( 'button' );
			close_button.className = close_button_class_name;
			...
			image.src =  wc_composite_admin_params.restriction.lock_img_src;
			image.alt = 'Locked';

			// Create the message
			const paragraph = document.createElement( 'p' );
			paragraph.textContent = self.options.message;

			// Create the call-to-action button
			const call_to_action = document.createElement( 'a' );
			call_to_action.href = self.options.cta_url;
			call_to_action.className = 'button button-primary button-large';
			call_to_action.textContent = self.options.cta_text;
			call_to_action.target = '_blank';
			call_to_action.onclick = function() {
				self.is_cta_navigating = true;
				call_to_action.classList.add( 'button-disabled' );
				call_to_action.disabled = true;
				if ( self.options.on_click_cta instanceof Function ) {
					self.options.on_click_cta();
				}
			}

			// Append all elements to their respective parents
			popover_content.appendChild( close_button );
			popover_content.appendChild( image );
			popover_content.appendChild( paragraph );
			popover_content.appendChild( call_to_action );

			self.overlay.appendChild( popover_content );
		}

		this.restrict = function() {
			self.create_popup_element();
			const attachment = $( self.options.attachment_selector );
			if (attachment.length) {
				attachment.prepend( self.overlay );
				if ( self.options.setup_show_trigger instanceof Function ) {
					self.options.setup_show_trigger( self );
				}
				if ( self.options.setup_hide_trigger instanceof Function ) {
					self.options.setup_hide_trigger( self );
				}
			}
		}

		this.hide = function() {
			if ( self.options.on_hide instanceof Function ) {
		...
		this.change_offset_top = function( offset ) {
			self.overlay.style.top = offset;
		}

		this.change_offset_left = function( offset ) {
			self.overlay.style.left = offset;
		}
	}

	function restrict_categories() {
		const on_show_record_event = _.once( function() {
			WC_CP.tracks.recordEvent( 'product_restriction_shown', {
				source: 'category',
				cta_tracks_event: wc_composite_admin_params.restriction.tracks_event,
			} );
		});

		const on_click_cta_record_event = function() {
			if ( wc_composite_admin_params.restriction.tracks_event ) {
				WC_CP.tracks.recordEvent( wc_composite_admin_params.restriction.tracks_event, {
					source: 'category',
				} );
			}
		};

		category_restriction = new Select2_Restriction(
			{
				restricted_option_key: 'category_ids',
				restricted_option_class: 'component_query_type',
				parent_initialize_event: 'wc-cp-component-initialized',
				parent_container: $components_panel,
				popup: {
					message: wc_composite_admin_params.restriction.i18n_message,
					cta_text: wc_composite_admin_params.restriction.i18n_cta_text,
					cta_url: wc_composite_admin_params.restriction.cta_url,
					on_click_cta: on_click_cta_record_event,
					on_show: on_show_record_event,
				}
			}
		);
		category_restriction.restrict();

		$components_panel.on( 'wc-cp-component-initialized', function( event, item ) {
			// We only show restriction notice if they already have it selected before.
			if ( 'category_ids' === item.$query_type_selector.val() ) {
				new Notice_Restriction(
					{
						parent_elements: [ item.$content.find('select.component_query_type').parent() ],
						message: wc_composite_admin_params.restriction.i18n_message_with_cta,
						on_click_cta: on_click_cta_record_event,
						on_show: on_show_record_event,
					}
				).show();
	...
	function restrict_scenario() {
		// Record once.
		const record_event_scenario_restriction_shown = function() {
			WC_CP.tracks.recordEvent( 'product_restriction_shown', {
				source: 'scenario',
				cta_tracks_event: wc_composite_admin_params.restriction.tracks_event,
			} );
		};
		$scenarios_tab_a_new.one( 'click', record_event_scenario_restriction_shown );

		const record_event_cta_clicked = function() {
			if ( wc_composite_admin_params.restriction.tracks_event ) {
				WC_CP.tracks.recordEvent( wc_composite_admin_params.restriction.tracks_event, {
					source: 'scenario',
				} );
		...
		if ( $scenarios.length === 0 ) {
			new Overlay_Restriction(
				{
					blurred_elements: $( '#bto_scenario_data .scenarios_config_group' ).get(),
					parent_elements: $( '#bto_scenario_data' ).get(),
					cta_text: wc_composite_admin_params.restriction.i18n_cta_text,
					cta_url: wc_composite_admin_params.restriction.cta_url,
					message: wc_composite_admin_params.restriction.i18n_message,
					on_click_cta: record_event_cta_clicked,
				}
			).show();
		} else {
			const notice_container_class = 'cp-restriction-notice-container';
			const notice_container = document.createElement( 'div' );
			notice_container.classList.add( 'toolbar' );
			notice_container.classList.add( notice_container_class );

			const first_toolbar = document.querySelector( '#bto_scenarios_inner .toolbar' );
			if ( first_toolbar ) {
				first_toolbar.after( notice_container );
				new Notice_Restriction( {
					parent_elements: document.querySelectorAll( '.' + notice_container_class ),
					message: wc_composite_admin_params.restriction.i18n_message_with_cta,
					on_click_cta: record_event_cta_clicked,
				} ).show();
			}
	...
	function restrict_layout_options() {
		new Overlay_Restriction( {
			blurred_elements: $( '.bto_layout_button.restricted_layout .layout_img, .bto_layout_button.restricted_layout .woocommerce-help-tip' ).get(),
			parent_elements: $( '.bto_layout_button.restricted_layout' ).get(),
		} ).show();

		// fired tracks event on popup or notice shown (once)
		const record_event_component_layout_shown = function() {
			if( record_event_component_layout_shown.fired ) return;
			WC_CP.tracks.recordEvent( 'product_restriction_shown', {
				source: 'component_layout',
				cta_tracks_event: wc_composite_admin_params.restriction.tracks_event,
			} );
			record_event_component_layout_shown.fired = true;
		};
		...
		const setup_popup = function( popup ) {
			$('.bto_layout_button.restricted_layout' ).on( 'click', function( e ) {
				const pos = $(this).position();
				popup.hide();
				popup.change_offset_left( ( pos.left + 43 ) + 'px');
				popup.change_offset_top( '-160px' );
				popup.show();
				record_event_component_layout_shown();
				e.preventDefault();
			});
		}

		// Clicking outside the popup or its trigger elements should hide it.
		const hide_popup = function( popup ) {
			$( document ).on( 'click', function( event ) {
				const whitelisted_classes = [
					'.cp-restriction-overlay-upsell-popover',
					'.cp-restriction-overlay-upsell',
					'.cp-restriction-overlay-upsell-popover *',
					'.cp-restriction-overlay-upsell *',
				]

				if ( $( event.target ).is( whitelisted_classes.join( ', ' )  ) ) {
					return;
				}
				popup.hide();
			});
		...
		new Popup_Restriction( {
			attachment_selector: '#woocommerce-product-data',
			message: wc_composite_admin_params.restriction.i18n_message,
			cta_text: wc_composite_admin_params.restriction.i18n_cta_text,
			cta_url: wc_composite_admin_params.restriction.cta_url,
			offset_top: 0,
			offset_left: 0,
			on_click_cta: record_event_cta_clicked,
			setup_show_trigger: setup_popup,
			setup_hide_trigger: hide_popup,
		} ).restrict();

		const saved_layout = $('input[name="bto_style"]:checked').val();

		if ( 'single' !== saved_layout ) {
			new Notice_Restriction( {
				parent_elements: $('.bto_layouts').get(),
				message: wc_composite_admin_params.restriction.i18n_message_with_cta,
				on_click_cta: record_event_cta_clicked,
				insert_strategy: 'append',
			} ).show();

			// when the notice restriction is shown, we fire the tracks event when the notice restriction is shown
			$( '.cp_components_tab' ).on('click', function() {
				record_event_component_layout_shown();
			});
		}
	}

	function notice_restriction_advanced_settings() {
		const record_event_on_show = function() {
			WC_CP.tracks.recordEvent( 'product_restriction_shown', {
				source: 'component_advanced_settings',
				cta_tracks_event: wc_composite_admin_params.restriction.tracks_event,
			} );
		};

		const record_event_on_click = function() {
			if ( wc_composite_admin_params.restriction.tracks_event ) {
				WC_CP.tracks.recordEvent( wc_composite_admin_params.restriction.tracks_event, {
					source: 'component_advanced_settings',
				} );
		...
		const notice_restriction = new Notice_Restriction(
			{
				parent_elements: $( '.tab_group_advanced' ).get(),
				message: wc_composite_admin_params.restriction.i18n_message_with_cta,
				on_click_cta: record_event_on_click,
			}
		);

		notice_restriction.show();
		$( '.bto_group_data' ).find( 'a[data-tab="advanced"]' ).one( 'click', record_event_on_show );
	}

	function restrict_component_advanced_settings( event, component ) {
		const record_event_on_show = function() {
			WC_CP.tracks.recordEvent( 'product_restriction_shown', {
				source: 'component_advanced_settings',
				cta_tracks_event: wc_composite_admin_params.restriction.tracks_event,
		...
		component.advanced_settings_restriction = new Overlay_Restriction(
			{
				blurred_elements: component.$el.find( '.tab_group_advanced .options_group_component' ).get(),
				parent_elements: component.$el.find('.tab_group_advanced' ).get(),
				cta_text: wc_composite_admin_params.restriction.i18n_cta_text,
				cta_url: wc_composite_admin_params.restriction.cta_url,
				message: wc_composite_admin_params.restriction.i18n_message,
				on_click_cta: record_event_on_click,
		...
		component.advanced_settings_restriction.show();
		component.$el.find( 'a[data-tab="advanced"]' ).one( 'click', record_event_on_show );
	}

	function is_restricted() {
		return wc_composite_admin_params.restriction.is_restricted || false;
	}
	...
	function init_restrictions() {
		...
		restrict_categories();
		restrict_scenario();
		restrict_layout_options();
		notice_restriction_advanced_settings();
		$components_panel.on( 'wc-cp-component-added', restrict_component_advanced_settings );
	}

	init_restrictions();

	$layout_options.on( 'click', function(e) {
		var $option = $( this );
		...
		// Highlight selected.
		$layout_options.removeClass( 'selected' );
		$layout_options.attr( 'aria-checked', false );
		$option.addClass( 'selected' );
		$option.attr( 'aria-checked', true );
	});

	$layout_options.on( 'focus', function() {
		var $option = $( this );
		$layout_options.attr( 'tabindex', -1 );
		$option.attr( 'tabindex', 0 );
	});
	$layout_options.on( 'keydown', function( e ) {
		if ( e.key === 'ArrowRight' || e.key === 'ArrowDown' ) {
			e.preventDefault();
			$( this ).next().click().focus();
		} else if ( e.key === 'ArrowLeft' || e.key === 'ArrowUp' ) {
