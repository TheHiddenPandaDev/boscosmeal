	...
	*/
	...
	public static function is_component_option_pricing_context( $product, $context = 'any' ) {
		...
		if ( in_array( $context, array( 'any', 'catalog' ) ) ) {
			...
			return ! is_null( WC_CP()->product_data->get( $product, 'composited_cart_item' ) );
		}
	}
	...
	public static function get_tax_ratios( $product ) {
		...
		$ref_price      = 1000.0;
		$ref_price_incl = (float) wc_get_price_including_tax(
			$product,
			array(
				'qty'   => 1,
				'price' => $ref_price,
		...
		$ref_price_excl = (float) wc_get_price_excluding_tax(
			$product,
			array(
				'qty'   => 1,
				'price' => $ref_price,
	...
	public static function get_discounted_price( $price, $discount ) {
		...
		if ( ! empty( $price ) && ! empty( $discount ) ) {
			$discounted_price = round( (float) $price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals( 'extended' ) );
		}
	...
	*/
		...
		if ( ! empty( $filtered_component_option ) ) {
			...
			foreach ( $prices_array['price'] as $variation_id => $price ) {
				...
				if ( $priced_per_product ) {
					...
					$price                   = empty( $discount ) ? $price : round( (float) $regular_price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals( 'extended' ) );
	...
	public static function filter_available_variation( $variation_data, $product, $variation ) {
		...
		if ( ! empty( $filtered_component_option ) ) {
			...
			$variation_data['availability'] = $filtered_component_option->get_availability()['availability'] ?? '';
	...
	public static function filter_get_price_html( $price_html, $product ) {
		...
		if ( ! empty( self::$filtered_component_option ) ) {
			...
			WC_CP()->product_data->set( $product, 'is_filtered_price_html', 'yes' );
	...
	public static function filter_get_price( $price, $product ) {
		...
		} elseif ( ! is_null( WC_CP()->product_data->get( $product, 'composited_cart_item' ) ) ) {
			$component_option = WC_CP()->product_data->get( $product, 'composited_cart_item' );
		}
		...
		if ( $component_option && ( $component_option instanceof WC_CP_Product ) ) {
			...
			if ( $discount = $component_option->get_discount() ) {
				...
				$offset_price     = ! is_null( WC_CP()->product_data->get( $product, 'composited_price_offset' ) ) ? WC_CP()->product_data->get( $product, 'composited_price_offset' ) : false;
				$offset_price_pct = ! is_null( WC_CP()->product_data->get( $product, 'composited_price_offset_pct' ) ) ? WC_CP()->product_data->get( $product, 'composited_price_offset_pct' ) : false;
