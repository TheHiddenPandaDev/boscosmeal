...
class WC_CP_Order {
	...
	public function add_composite_to_order( $composite, $order, $quantity = 1, $args = array() ) {
		...
		if ( $composite && 'composite' === $composite->get_type() ) {
			...
			try {
				...
				$is_valid      = WC_CP()->cart->validate_composite_configuration(
					$composite,
					$quantity,
					$configuration,
					array(
						...
						if ( $component_option->is_priced_individually() ) {
							if ( $component_discount ) {
								$component_args['subtotal']     = isset( $component_args['subtotal'] ) ? $component_args['subtotal'] : wc_get_price_excluding_tax(
									$component_option_product,
									array(
										'qty'   => $component_quantity * $quantity,
										'price' => $component_option->is_discount_allowed_on_sale_price() ? $component_option_product->get_price() : $component_option_product->get_regular_price(),
									)
								) * ( 1 - (float) $component_discount / 100 );
								$component_args['total']        = isset( $component_args['total'] ) ? $component_args['total'] : wc_get_price_excluding_tax(
									$component_option_product,
									array(
										'qty'   => $component_quantity * $quantity,
										'price' => $component_option->is_discount_allowed_on_sale_price() ? $component_option_product->get_price() : $component_option_product->get_regular_price(),
	...
	*/
	...
	public function container_item_needs_processing( $is_needed, $product, $order_id ) {
		...
		$composite_needs_processing = WC_CP()->product_data->get( $product, 'composite_needs_processing' );

		if ( $product->is_type( 'composite' ) && ! is_null( $composite_needs_processing ) && 'no' === $composite_needs_processing ) {
			$is_needed = false;
		}
			...
			if ( $child_items = wc_cp_get_composited_order_items( $item ) ) {
				...
				if ( $product->is_virtual() && sizeof( $child_items ) > 0 ) {
					if ( 'no' === $item->get_meta( '_components_need_processing', true ) ) {
						WC_CP()->product_data->set( $product, 'composite_needs_processing', 'no' );
					}
				}
			}
		}
