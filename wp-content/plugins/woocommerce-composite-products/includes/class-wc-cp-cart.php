	...
	*/
		...
		if ( $cart_item['data']->needs_shipping() ) {
			...
			if ( false === $component_option->is_shipped_individually() ) {
				...
				if ( $component_option->is_weight_aggregated( $cart_item['data'] ) ) {
					...
					WC_CP()->product_data->set( $cart_item['data'], 'composited_weight', $cart_item_weight );
				}

				$composited_value = 'props' === WC_CP_Products::get_composited_cart_item_discount_method() ? $cart_item['data']->get_price( 'edit' ) : $component_option->get_raw_price( $cart_item['data'], 'cart' );
	...
	private function set_composited_cart_item_price( $cart_item, $component_id, $composite ) {
		...
		if ( 'filters' === $discount_method ) {
			...
			WC_CP()->product_data->set( $cart_item['data'], 'composited_cart_item', $component_option );
	...
	*/
	...
	public function get_cart_item_from_session( $cart_item, $item_session_values ) {
		...
		if ( wc_cp_is_composite_container_cart_item( $item_session_values ) ) {
			...
			} elseif ( isset( $cart_item['composite_children'] ) ) {
	...
	public function cart_item_remove( $cart_item_key, $cart ) {
		...
		if ( wc_cp_is_composite_container_cart_item( $cart->removed_cart_contents[ $cart_item_key ] ) ) {
			...
			WC()->cart->cart_contents[ $container_key ]['composite_children'] = array_values( array_diff( $cart->cart_contents[ $container_key ]['composite_children'], array( $cart_item_key ) ) );
		}
	}
						...
						if ( wc_cp_is_composite_container_cart_item( $cart_item_data ) ) {
							...
							if ( $composite->needs_shipping() ) {
								...
								foreach ( wc_cp_get_composited_cart_items( $cart_item_data, WC()->cart->cart_contents, true ) as $child_item_key ) {
									...
									$composited_product_value  = ! is_null( WC_CP()->product_data->get( $composited_product, 'composited_value' ) ) ? WC_CP()->product_data->get( $composited_product, 'composited_value' ) : 0.0;
									$composited_product_weight = ! is_null( WC_CP()->product_data->get( $composited_product, 'composited_weight' ) ) ? WC_CP()->product_data->get( $composited_product, 'composited_weight' ) : 0.0;
	...
	public function sold_individually_found_in_cart( $found, $product_id, $variation_id, $cart_item ) {
		...
		foreach ( WC()->cart->get_cart() as $search_cart_item ) {
			if ( $product_id === $search_cart_item['product_id'] && 'product' === $product->get_sold_individually_context() ) {
				...
				$button_class    = wc_cp_wp_theme_get_element_class_name( 'button' );
