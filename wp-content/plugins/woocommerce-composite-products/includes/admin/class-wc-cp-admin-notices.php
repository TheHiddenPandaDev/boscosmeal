...
class WC_CP_Admin_Notices {
	...
	public static function init() {
		...
		self::$maintenance_notices = is_array( get_option( 'wc_cp_maintenance_notices' ) ) ? get_option( 'wc_cp_maintenance_notices' ) : array();
	...
	public static function welcome_notice() {
		...
		<p class="sw-welcome-text">
			<?php
				...
				echo wp_kses_post( sprintf( __( 'Thank you for installing <strong>WooCommerce Composite Products</strong>. Ready to get started? <a href="%s">Click here to create your first composite product</a>.', 'woocommerce-composite-products' ), admin_url( 'post-new.php?post_type=product&wc_cp_first_composite=1&tutorial=true&tutorial_type=composite-products' ) ) );
			?>
		</p>
		<?php
