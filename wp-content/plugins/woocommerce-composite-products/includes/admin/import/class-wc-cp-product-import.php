			...
			try {
				...
				if ( isset( $data['wc_cp_components'] ) ) {
					...
					if ( ! empty( $data['wc_cp_components'] ) ) {
						...
						foreach ( $data['wc_cp_components'] as $key => $component_data ) {
							...
							$component_data = WC_CP_REST_API::sanitize_rest_api_component_data( $component_data );
							...
							if ( $thumbnail_id || $thumbnail_src ) {
								$component_data['thumbnail_id'] = WC_CP_Component::set_thumbnail( $thumbnail_id, $thumbnail_src, $product );
							}
							...
							// Validate data.
							$component_data = WC_CP_REST_API::validate_internal_component_data( $component_data, 'import' );
							 ...
							 * @since 9.0.4
							 *
							 * @param  array   $component_data  Sanitized, converted and validated data to be passed to the REST API.
							 * @param  string  $action          Create.
							 * @param  array   $data            Source data passed from the CSV $data[ 'wc_cp_components' ][ $key ] .
							 * @param  int     $product_id      Composite product ID $product->get_id().
							 */
							$composite_data[ $component_id ] = (array) apply_filters( 'woocommerce_composite_products_rest_api_update_component_data', $component_data, $action, $data['wc_cp_components'][ $key ], $product->get_id() );
