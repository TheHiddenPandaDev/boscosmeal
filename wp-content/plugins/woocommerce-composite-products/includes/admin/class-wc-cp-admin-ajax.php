...
class WC_CP_Admin_Ajax {
	...
	*/
	...
	public static function ajax_composite_order_item_form() {
		...
		ob_start();
		/**
		 * 'woocommerce_composite_before_order_item_form' action.
		 *
		 * @since 10.1.0
		 ...
		 */
		do_action( 'woocommerce_composite_before_order_item_form', $product, $item, $order );
		include WC_CP_ABSPATH . 'includes/admin/meta-boxes/views/html-composite-edit-form.php';
		/**
		 * 'woocommerce_composite_after_order_item_form' action.
		 *
		 * @since 10.1.0
		 * @param WC_Product_Composite $product
		 * @param WC_Order_Item $item
		 * @param WC_Order $order
		 */
		do_action( 'woocommerce_composite_after_order_item_form', $product, $item, $order );
		$html = ob_get_clean();
	...
	public static function ajax_edit_composite_in_order() {
		...
		try {
			/**
			 * 'woocommerce_before_editing_composite_in_order' action.
			 *
			 * @since  10.1.0
			 *
			 ...
			 * @param WC_Order               $order
			 */
			do_action( 'woocommerce_before_editing_composite_in_order', $item, $order );
		} catch ( Exception $e ) {
			wp_send_json( $failure );
		}
		 ...
		 */
		$posted_configuration  = apply_filters( 'woocommerce_editing_composite_in_order_configuration', WC_CP()->cart->get_posted_composite_configuration( $product ), $product, $item, $order );
		...
		if ( $posted_configuration !== $current_configuration ) {
			...
			if ( is_wp_error( $added_to_order ) ) {
				...
				if ( $notice ) {
					...
					$message = sprintf( _x( '%1$s %2$s', 'edit composite in order: formatted validation message', 'woocommerce-composite-products' ), $message, html_entity_decode( $notice, ENT_COMPAT ) );
				}
		...
		// Reload the order to ensure item meta-data is up-to-date.
