...
class WC_CP_Meta_Box_Product_Data {
	...
	public static function maybe_add_metabox_load_notices() {
		...
		if ( 'product' !== $screen_id || isset( $_GET['tutorial'] ) ) {
			return;
		}
	...
	public static function maybe_add_states_migration_notice( $product ) {
		...
		if ( self::get_global_object_states_data( $product, 'needs_migration' ) ) {
			...
			WC_CP_Admin_Notices::add_notice(
				$notice,
				array(
					'dismiss_class' => 'cp_states',
					'type'          => 'info',
				)
			);
		}
	...
	public static function composite_layout( $composite_product_object ) {
		$restricted_layout_class = WC_CP_Admin_Restrictions::is_restricted( WC_CP()->wccom_product_id ) ? 'restricted_layout' : '';
		...
		?><div class="bundle_group bto_clearfix">
			<div class="bto_layouts bto_clearfix form-field components_panel_field">
				...
				<div role="radiogroup" class="bto_clearfix bto_layouts_list" aria-label="<?php esc_attr_e( 'Layout', 'woocommerce-composite-products' ); ?>">
					<?php
					...
					foreach ( $layouts as $layout_id => $layout_data ) {
						...
						$selected_class = $selected_layout === $layout_id ? 'selected' : '';
						// We don't show the lock icon if the option is selected, we still allow them to edit it.
						$restricted_class = ( 'selected' === $selected_class ) || ( 'single' === $layout_id ) ? '' : $restricted_layout_class;

						?>
						<button role="radio" type="button" class="bto_layout_button <?php echo esc_attr( $selected_class . ' ' . $restricted_class ); ?>" tabindex="<?php echo $selected_class ? '0' : '-1'; ?>" aria-label="<?php echo esc_attr( $layout_data['title'] ); ?>" aria-description="<?php echo esc_attr( $layout_data['description'] ); ?>">
							<label class="bto_layout_label">
	...
	public static function shipping_type_admin_html() {
		...
		<div class="options_group composite_type show_if_composite">
			<div class="form-field">
				<label id="bto_composite_type_label"><?php esc_html_e( 'Composite type', 'woocommerce-composite-products' ); ?></label>
				<div role="radiogroup" class="bto_type_options" aria-labelledby="bto_composite_type_label">
					<?php
		...
		<div class="sw-form-os">
			<div class="os_container widefat wc-cp-<?php echo esc_attr( $field_type ); ?>-conditions-container <?php echo $conditions_count ? '' : 'os_empty'; ?>" data-os_count="<?php echo (int) $conditions_count; ?>" data-os_post_name="<?php echo esc_attr( $post_name ); ?>" data-unique_additions="yes">
				<div class="os_boarding<?php echo $conditions_count ? '' : ' active'; ?>">
					...
					if ( 'state' === $field_type ) {
						?>
							<div class="text"><?php esc_html_e( 'First, add some Components.', 'woocommerce-composite-products' ); ?></br><?php esc_html_e( 'Then, choose products or variations that can be bought together.', 'woocommerce-composite-products' ); ?></div>
							<?php
					} else {
						?>
							<div class="text"><?php esc_html_e( 'First, add some Conditions.', 'woocommerce-composite-products' ); ?></br><?php esc_html_e( 'Then, choose an Action to trigger when these Conditions are satisfied.', 'woocommerce-composite-products' ); ?></div>
							<?php
					}
					?>
				</div>
	...
	public static function component_select_action_options( $id, $data, $product_id ) {
		...
		<div class="component_select_action">
			<div class="form-field">
				...
				<select class="sw-select2" data-wrap="-tipped" style="width: 100%" name="bto_data[<?php echo esc_attr( $id ); ?>][select_action]">
																											<?php
	...
	public static function component_options_group_pre_sf( $id, $data, $product_id ) {
		...
		?>
		<div class="options_group options_group_component options_group_component--sort-filter" <?php echo $show_sf_options ? '' : 'style="display:none"'; ?>>
		<?php
	}
	...
	public static function component_config_image( $id, $data, $product_id ) {
		...
		<div class="component_image group_image">
			<div class="form-field">
				...
				<a href="#" class="upload_component_image_button <?php echo $image_id ? 'has_image' : ''; ?>"><span class="prompt"><?php esc_html_e( 'Select image', 'woocommerce-composite-products' ); ?></span><img src="
																			<?php
																			if ( ! empty( $image ) ) {
																				echo esc_attr( $image );
																			} else {
																			...
																			?>
																" /><input type="hidden" name="bto_data[<?php echo esc_attr( $id ); ?>][thumbnail_id]" class="image" value="<?php echo esc_attr( $image_id ); ?>" /></a>
	...
	public static function component_config_options( $id, $data, $product_id ) {
		...
		<div class="component_query_type"<?php echo $options_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			<div class="form-field">
				...
				<select class="component_query_type sw-select2" data-wrap="-tipped" name="bto_data[<?php echo esc_attr( $id ); ?>][query_type]" style="width: 100%;">
																												<?php
		...
		<div class="component_selector bto_selector component_query_type_selector bto_multiselect component_query_type_product_ids">
			<div class="form-field"><select id="bto_ids_<?php echo esc_attr( $id ); ?>" class="sw-select2-search--products products_selector" data-wrap="-tipped" name="bto_data[<?php echo esc_attr( $id ); ?>][assigned_ids][]" multiple="multiple" style="width: 100%;" data-limit="100" data-action="woocommerce_json_search_component_options" data-placeholder="<?php echo esc_attr__( 'Search for a product&hellip;', 'woocommerce' ); ?>" data-sortable="true">
				<?php
		...
		<div class="component_category_selector bto_category_selector component_query_type_selector bto_multiselect component_query_type_category_ids">
			<div class="form-field">
				...
				<select id="bto_category_ids_<?php echo esc_attr( $id ); ?>" class="multiselect sw-select2 categories_selector" data-wrap="-tipped" name="bto_data[<?php echo esc_attr( $id ); ?>][assigned_category_ids][]" style="width: 100%" multiple="multiple" data-placeholder="<?php echo esc_attr__( 'Select categories&hellip;', 'woocommerce-composite-products' ); ?>">
														<?php
		...
		<div class="component_default_selector default_selector_container" data-selected_data="<?php echo wc_esc_json( json_encode( $selected_data ) ); ?>">
			<div class="form-field">
				...
				<div class="component_query_type_product_ids default_selector_wrapper">
					<select id="group_product_ids_default_<?php echo esc_attr( $id ); ?>" class="sw-select2 default_selector_products" data-wrap="-tipped" style="width: 100%;" name="bto_data[<?php echo esc_attr( $id ); ?>][default_id_products]" data-allow_clear="true" data-placeholder="<?php esc_attr_e( 'Choose a product&hellip;', 'woocommerce-composite-products' ); ?>">
																	<?php
	...
	public static function component_config_display_prices( $id, $data, $product_id ) {
		...
		<div class="component_display_prices">
			<div class="form-field">
				...
				<select class="sw-select2" data-wrap="-tipped" style="width: 100%" name="bto_data[<?php echo esc_attr( $id ); ?>][display_prices]">
																											<?php
	...
	public static function component_config_options_style( $id, $data, $product_id ) {
		...
		<div class="component_options_style group_options_style" <?php echo $show_options_style ? '' : 'style="display:none;"'; ?>>
			<div class="form-field">
				...
				<select class="options_style_selector sw-select2" data-wrap="-tipped" name="bto_data[<?php echo esc_attr( $id ); ?>][selection_mode]" style="width: 100%;" >
																												<?php
	...
	public static function component_config_pagination_style( $id, $data, $product_id ) {
		...
		<div class="component_pagination_style" <?php echo $show_pagination_style ? '' : 'style="display:none;"'; ?>>
			<div class="form-field">
				...
				<select class="sw-select2" data-wrap="-tipped" style="width: 100%" name="bto_data[<?php echo esc_attr( $id ); ?>][pagination_style]">
																											<?php
				...
				<div class="options_group states_config_group bto_clearfix <?php echo empty( $states_data ) ? 'options_group--boarding' : ''; ?>">
					...
					if ( ! empty( $composite_data ) ) {
						...
						?>
							...
							<div class="bto_states wc-metaboxes">
							<?php
	...
	public static function add_composite_type_options( $options ) {
		...
		if ( empty( $product_object ) || false === $product_object->is_type( 'composite' ) ) {
			$composite_product_object = $post instanceof WP_Post ? new WC_Product_Composite( $post->ID ) : new WC_Product_Composite();
		} else {
	...
	public static function process_posted_composite_configuration( $product, $posted_composite_data = array() ) {
		...
		if ( isset( $posted_composite_data['bto_data'] ) ) {
			...
			foreach ( $posted_composite_data['bto_data'] as $row_id => $post_data ) {
				...
				} elseif ( isset( $post_data['pagination_style'] ) && in_array( $post_data['pagination_style'], wp_list_pluck( WC_CP_Component::get_pagination_style_options(), 'id' ) ) && ! in_array( $props['layout'], array( 'single', 'progressive' ) ) ) {
			...
			if ( isset( $posted_scenario_data ) ) {
				...
				foreach ( $posted_scenario_data as $scenario_post_data ) {
					...
					if ( $copied_from_current ) {
						...
						} elseif ( isset( $scenario_data[ $scenario_id ]['scenario_actions'] ) && is_array( $scenario_data[ $scenario_id ]['scenario_actions'] ) ) {
			...
			if ( ! empty( $ordered_scenario_data ) ) {
				...
				$scenarios_manager = new WC_CP_Scenarios_Manager(
					array(
	...
	public static function add_notice( $content, $type ) {
		...
		if ( 'warning' !== $type ) {
			self::$ajax_notices[] = strip_tags( html_entity_decode( $content, ENT_COMPAT ) );
		}
	}
			...
			<div class="os_value">
				<div data-modifiers="in,not-in"<?php echo in_array( $modifier, array( 'in', 'not-in' ) ) ? '' : ' style="display:none;"'; ?>>
					...
					if ( false === $use_ajax ) {
						...
						?>
						<select name="<?php echo esc_attr( $post_name ); ?>[component_data][<?php echo esc_attr( $component_id ); ?>][]" style="width: 100%;" class="sw-select2 bto_scenario_ids" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select products and variations&hellip;', 'woocommerce-composite-products' ); ?>">
						<?php
						...
						?>
						<select name="<?php echo esc_attr( $post_name ); ?>[component_data][<?php echo esc_attr( $component_id ); ?>][]" class="sw-select2-search--products" multiple="multiple" style="width: 100%;" data-include="
												<?php
													echo wc_esc_json(
														json_encode(
															array(
												...
												?>
													" data-limit="100" data-action="woocommerce_json_search_products_and_variations_in_component" data-action_version="8.0" data-placeholder="<?php echo esc_attr__( 'Search for products and variations&hellip;', 'woocommerce-composite-products' ); ?>">
						<?php
	...
	public static function print_conditions_js_templates() {
		...
		?>
		<script type="text/template" id="tmpl-wc_cp_flat_condition_content">
		<?php
		...
		// Generating for {{{ data.os_content }}}.
		?>
		<script type="text/template" id="tmpl-wc_cp_ajax_condition_content">
		<?php
	...
	public static function print_condition_components_dropdown( $composite_data, $selected_id = false, $additional_options = array() ) {
		...
		foreach ( $composite_data as $component_id => $component_data ) {
			...
			?>
				<option value="<?php echo esc_attr( $component_id ); ?>" <?php echo $component_id === $selected_id ? 'selected="selected"' : ''; ?>>
				<?php
