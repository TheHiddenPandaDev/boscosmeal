...
class WC_Product_Composite extends WC_Product {
	...
	/**
	 * Composite-specific add to cart form settings.
	 *
	 * @var array
	 */
	...
	/**
	 * True if Composite is a NYP product.
	 *
	 * @var boolean
	 */
	private $is_nyp;
	...
	/**
	 * Scenarios manager view.
	 *
	 * @var mixed
	 */
	public $scenarios_manager_view;

	/**
	 * Total number of component options.
	 *
	 * @var int
	 */
			...
			if ( 'completed' === $price_data['status'] ) {
				...
				foreach ( $components as $component_id => $component ) {
					...
					if ( $min_component_raw_price_option ) {
						...
						$quantity_min            = $component->is_optional() && 'defaults' !== $this->get_shop_price_calc() ? 0 : $component->get_quantity( 'min' );
	...
	public function calculate_price( $args ) {
		...
		if ( $this->contains( 'priced_individually' ) ) {
			...
			if ( isset( $this->composite_price_cache[ $cache_key ] ) ) {
				...
				if ( '' === $this->$raw_price_fn() || INF === $this->$raw_price_fn() || empty( $this->permutations[ $min_or_max ] ) ) {
					...
					$price    = wc_format_decimal(
						WC_CP_Products::get_product_price(
							$this,
							array(
								'price' => $this->$price_fn(),
								'qty'   => $qty,
					...
					foreach ( $this->permutations[ $min_or_max ] as $component_id => $product_id ) {
						...
						$item_qty  = 'min' === $min_or_max && $component->is_optional() && 'defaults' !== $this->get_shop_price_calc() ? 0 : $qty * $component->get_quantity( $min_or_max );
			...
			$price_fn = 'get_' . $price_prop;
			$price    = WC_CP_Products::get_product_price(
				$this,
				array(
					'price' => $this->$price_fn(),
					'qty'   => $qty,
	...
	public function add_to_cart_form_settings() {
		...
		$settings = array(
			...
			'sequential_componentized_progress'      => apply_filters( 'woocommerce_composite_sequential_comp_progress', 'no', $this ), /*
		yes | no */
	...
	*/
	...
	public function get_shop_price_calc( $context = 'any' ) {
		...
		return $this->get_prop( 'shop_price_calc', $context );
	}
	...
	public function get_composite_data( $context = 'any' ) {
		...
		foreach ( $components as $component_id => $component ) {
			...
			if ( 'rest' === $context ) {
				...
				/**
				 * Filters the component data on REST API get context.
				 *
				 * @since 9.0.2
				 *
				 * @param  array         $rest_component_data   Component data to be passed to the REST API.
				 * @param  array|object  $component             Component object.
				 * @param  int           $composite_product_id  Composite product ID.
				 */
				$composite_data[ $component_id ] = (array) apply_filters( 'woocommerce_composite_products_rest_api_get_component_data', $rest_component_data, $component, $this->get_id() );
