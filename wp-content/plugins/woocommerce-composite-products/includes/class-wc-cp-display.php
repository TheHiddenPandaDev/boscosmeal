...
class WC_CP_Display {
	...
	public function frontend_scripts() {
		...
		$params = apply_filters(
			...
			array(
				...
				'i18n_summary_add_ons_prompt'              => __( 'Add-ons', 'woocommerce-composite-products' ),
	...
	*/
	...
	public function get_container_cart_item_price_amount( $cart_item, $type ) {
		...
		$composite_price   = WC_CP_Products::get_product_price(
			$cart_item['data'],
			array(
		...
		foreach ( $child_cart_items as $child_cart_item ) {
			...
			$child_item_price   = WC_CP_Products::get_product_price(
				$child_cart_item['data'],
				array(
					'price' => $child_cart_item['data']->$price_fn(),
					'calc'  => $calc_type,
					'qty'   => $child_item_qty,
				)
			);
		...
		$price = (float) $composite_price + $child_items_price;
	...
	public function get_container_cart_item_subtotal_amount( $cart_item, $type ) {
		...
		$child_items_price = 0.0;
		$composite_price   = wc_format_decimal( (float) $cart_item[ 'line_' . $type ], wc_cp_price_num_decimals() );
		$child_cart_items  = wc_cp_get_composited_cart_items( $cart_item, WC()->cart->cart_contents, false, true );
	...
	*/
	...
	public function order_item_subtotal( $subtotal, $item, $order ) {
		...
		if ( $parent_item = wc_cp_get_composited_order_item_container( $item, $order ) ) {
			...
			if ( is_null( WC_CP()->product_data->get( $item, 'child_subtotals_added' ) ) ) {
				...
				if ( $aggregate_subtotals ) {
					...
					if ( ! empty( $children ) ) {
						...
						WC_CP()->product_data->set( $cloned_item, 'child_subtotals_added', 'yes' );
	...
	public function order_item_component_name( $content, $order_item ) {
		...
		if ( false !== $this->order_item_order && wc_cp_is_composited_order_item( $order_item, $this->order_item_order ) ) {
			...
			if ( is_checkout_pay_page() || is_order_received_page() || is_view_order_page() || ( function_exists( 'wcs_is_view_subscription_page' ) && wcs_is_view_subscription_page() ) ) {
				...
				if ( ! empty( $composited_item_data['attributes'] ) ) {
					...
					foreach ( $composited_item_data['attributes'] as $attribute_name => $attribute_value ) {
						...
						if ( taxonomy_exists( $taxonomy ) ) {
							...
							$attribute_value    = apply_filters( 'woocommerce_variation_option_name', $attribute_value, null, $attribute_name, $composited_product );
