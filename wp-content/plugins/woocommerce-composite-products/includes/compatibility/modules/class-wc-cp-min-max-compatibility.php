...
class WC_CP_Min_Max_Compatibility {
	...
	public static function save_quantity_input_args( $data, $product ) {
		...
		if ( is_object( self::$composited_item ) || ! is_null( WC_CP()->product_data->get( $product, 'wc_mmq_composited_item' ) ) ) {
	...
	public static function restore_quantity_input_args( $data, $product ) {
		...
		if ( is_array( self::$unfiltered_args ) ) {
			...
			if ( is_a( $product, 'WC_Product' ) ) {
				$mmq_composited_item = WC_CP()->product_data->get( $product, 'wc_mmq_composited_item' );
			} else {
				// See self::composited_variation_data(), which calls this function with 'false' as the product.
				$mmq_composited_item = null;
			}

			if ( is_null( $mmq_composited_item ) ) {
				...
				// Cart context.
				$group_of_qty = self::get_composited_item_group_of_qty( $mmq_composited_item ) * WC_CP()->product_data->get( $product, 'container_quantity' );
		...
		if ( wc_cp_is_composited_cart_item( $cart_item ) ) {
			...
			if ( $composited_container_item = wc_cp_get_composited_cart_item_container( $cart_item ) ) {
				...
				if ( 'composite' === $composite->get_type() && $composite->has_component( $cart_item['composite_item'] ) ) {
					WC_CP()->product_data->set( $product, 'wc_mmq_composited_item', $cart_item['data'] );
					WC_CP()->product_data->set( $product, 'container_quantity', $composited_container_item['quantity'] );
				}
			}
		}
