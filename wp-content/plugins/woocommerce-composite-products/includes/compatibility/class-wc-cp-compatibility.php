...
class WC_CP_Compatibility {
	...
	public function module_includes() {
		...
		if ( class_exists( '\WooCommerce\PayPalCommerce\PluginModule' ) ) {
			$module_paths['paypal'] = WC_CP_ABSPATH . 'includes/compatibility/modules/class-wc-cp-paypal-compatibility.php';
		}

		// WC Payments compatibility
		if ( class_exists( 'WC_Payments' ) ) {
			$module_paths['wcpay'] = WC_CP_ABSPATH . 'includes/compatibility/modules/class-wc-cp-wc-payments-compatibility.php';
		}
	...
	public function check_required_versions() {
		...
		if ( class_exists( 'WC_Bundles' ) && function_exists( 'WC_PB' ) ) {
			...
			if ( version_compare( WC_CP()->plugin_version( true, WC_PB()->version ), $required_version ) < 0 ) {
				...
				WC_CP_Admin_Notices::add_dismissible_notice(
					$notice,
					array(
		...
		if ( class_exists( 'WC_CP_Conditional_Images' ) ) {
			...
			if ( version_compare( WC_CP()->plugin_version( true, WC_CP_Conditional_Images::$version ), $required_version ) < 0 ) {
				...
				WC_CP_Admin_Notices::add_dismissible_notice(
					$notice,
					array(
						'dismiss_class' => 'cp_ci_' . $required_version,
						'type'          => 'warning',
					)
				);
			}
		...
		if ( class_exists( 'WC_Min_Max_Quantities' ) && defined( 'WC_MIN_MAX_QUANTITIES' ) ) {
			...
			if ( version_compare( WC_MIN_MAX_QUANTITIES, $required_version ) < 0 ) {
				...
				WC_CP_Admin_Notices::add_dismissible_notice(
					$notice,
					array(
						'dismiss_class' => 'mmq_lt_' . $required_version,
						'type'          => 'warning',
					)
				);
			}
