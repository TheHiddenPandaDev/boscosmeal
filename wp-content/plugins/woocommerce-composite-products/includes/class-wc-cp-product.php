...
class WC_CP_Product {
	...
	/**
	 * True if currently renewing a subscription.
	 *
	 * @var boolean
	 */
			...
			if ( $this->is_priced_individually() ) {
				...
				if ( in_array( $product_type, array( 'simple', 'variation' ) ) ) {
					...
					if ( $discount = $this->get_discount() ) {
						$this->min_price = empty( $min_regular_price ) ? $min_regular_price : round( (float) $min_regular_price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals( 'extended' ) );
						$this->max_price = empty( $max_regular_price ) ? $max_regular_price : round( (float) $max_regular_price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals( 'extended' ) );
					} else {
	...
	public function get_raw_price( $product = false, $context = '' ) {
		...
		if ( $discount = $this->get_discount() ) {
			$price = empty( $regular_price ) ? $regular_price : round( (float) $regular_price * ( 100 - $discount ) / 100, wc_cp_price_num_decimals( 'extended' ) );
		}
	...
	private function get_availability_text( $product = false ) {
		...
		} elseif ( class_exists( 'WC_CP_Admin_Ajax' ) && WC_CP_Admin_Ajax::is_composite_edit_request() ) {
