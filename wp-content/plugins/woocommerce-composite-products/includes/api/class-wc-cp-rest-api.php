...
class WC_CP_REST_API {
	...
	public static function init() {
		...
		// Product template
		add_filter( 'woocommerce_product_template_csv_file_path', array( __CLASS__, 'composite_product_template_csv_file_path' ), 10, 2 );
	...
	*/
	...
	private static function get_extended_product_schema() {
		...
		return apply_filters(
			'woocommerce_composite_products_rest_api_product_schema',
			array(
					...
					if ( ! empty( $field_value ) && is_array( $field_value ) ) {
						foreach ( $field_value as $data ) {
							...
							if ( 'create' === $action ) {
								...
								/**
								 * Filters the component data on REST API update context.
								 *
								 * @since 9.0.2
								 *
								 ...
								 */
								$new[ $component_id ] = (array) apply_filters( 'woocommerce_composite_products_rest_api_update_component_data', $component_data, $action, $data, $product_id );
								 ...
								 * @param  int     $product_id      Composite product ID.
								 */
								$updated[ $component_id ] = (array) apply_filters( 'woocommerce_composite_products_rest_api_update_component_data', $component_data, $action, $data, $product_id );
							}
						}
	...
	*/
	...
	/**
	 * If the requested template is 'composite-product', return the path to the composite product template.
	 *
	 * @since 9.0.2
	 ...
	 * @return string
	 */
	public static function composite_product_template_csv_file_path( $template_path, $template_name ) {
		if ( 'composite-products' === $template_name ) {
			$template_path = WC_CP_ABSPATH . 'includes/api/templates/composite-product-template.csv';
		}
