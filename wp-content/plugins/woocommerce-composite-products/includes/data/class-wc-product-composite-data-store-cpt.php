	...
	public function read_price_data( &$product, $task_runner_args = false ) {
			...
			if ( $permutations_calc_scenarios ) {
				...
				*/
				...
				foreach ( $component_option_prices as $component_id => $component_option_price_data ) {
					...
					$price_data['min'][ $component_id ] = $component->is_optional() && 'defaults' !== $product->get_shop_price_calc() || 0 === $quantity_min ? 0 : $min_component_price_id;
		...
		if ( $resume_from_permutation ) {
			...
			if ( false === $task_runner_args ) {
				...
				WC_CP_Products::schedule_price_calc_task(
					array(
						'min'          => $price_data['min'],
						'max'          => $price_data['max'],
	...
	public function get_raw_component_option_prices( $product_ids ) {
		...
		if ( null === $results ) {
			...
			$results = $wpdb->get_results(
				...
				AND postmeta.post_id IN ( {$placeholders} )
			",
					...$expanded_ids
				),
				ARRAY_A
		...
		if ( class_exists( 'WC_Name_Your_Price_Helpers' ) && apply_filters( 'woocommerce_composite_read_nyp_component_option_prices', false ) ) {
			...
			if ( null === $nyp_results ) {
				...
				if ( ! empty( $nyp_ids ) ) {
					...
					$nyp_ids = array_map( 'absint', $nyp_ids );

					// This variable adds as many %d placeholders to the query as the IDs. Therefore, we are skipping PHPCS checks for this query.
					$placeholders = implode( ', ', array_fill( 0, count( $nyp_ids ), '%d' ) );
			...
			foreach ( $nyp_results as $nyp_result ) {
				...
				$price_min = '' === $nyp_result['min_price'] ? 0.0 : (float) $nyp_result['min_price'];
				$price_max = INF;
